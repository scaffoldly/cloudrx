...
> {
  ...
  protected _stream(_all: boolean): Observable<_Record[]> {
    throw new Error('Method not implemented.');
  }
  protected _store<T>(_item: T): Observable<Matcher<_Record>> {
    throw new Error('Method not implemented.');
  }
  protected _unmarshall<T>(_event: _Record): Streamed<T, unknown> {
    throw new Error('Method not implemented.');
  }
}
